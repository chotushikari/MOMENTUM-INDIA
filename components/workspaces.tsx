...
export function DeepDivePage({ video }: { video: ShortVideo }) {
  ...
  return <>
    ...
    <section className="deep-two-column"><div className="panel-surface evidence-table"><div className="section-heading"><div><p className="eyebrow">Observed details</p><h2>What the source says</h2></div></div><div className="detail-rows"><span>Channel</span><strong>{video.channel}</strong><span>Topic</span><strong>{video.topic}</strong><span>Format</span><strong>{video.format}</strong><span>Category</span><strong>{video.category}</strong><span>Source</span><strong>{video.sourceMode === "live" ? "YouTube API" : "Sample fixture"}</strong></div><p>{video.why}</p></div><aside className="panel-surface momentum-next"><p className="eyebrow">Extra layer</p><h2>What we add beyond a trend list</h2><div className="dna-list"><Dna label="Niche translation" text="Rewrite the signal for food, gaming, education, or creator-business niches." /><Dna label="Premium locks" text="City, competitor, export, creator fit, and 30-day history stay locked until backed by repeated evidence." /><Dna label="Action memory" text="Save the signal, turn it into ideas, and return to the same source context later." /></div></aside></section>
