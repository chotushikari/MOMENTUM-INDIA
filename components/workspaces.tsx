...
export function DeepDivePage({ video }: { video: ShortVideo }) {
  ...
  return <>
    ...
    <div className="deep-insight panel-surface">{tab === "why" && <><p className="eyebrow">AI interpretation / grounded in observed fields</p><h2>Why this is moving</h2>{insightError ? <p className="insight-lede">Grounded intelligence is temporarily unavailable. The observed evidence remains available above.</p> : <p className="insight-lede">{insight?.why ?? "Interpreting the observed evidence..."}</p>}<div className="insight-grid"><Insight label="Hook" value={insight?.hook ?? "Awaiting grounded interpretation"} /><Insight label="Format" value={insight?.format ?? video.format} /><Insight label="Viewer payoff" value={insight?.payoff ?? "Awaiting grounded interpretation"} /><Insight label="Evidence" value={`${video.views.toLocaleString()} views / ${video.likes.toLocaleString()} likes / ${video.comments.toLocaleString()} comments`} /></div></>}{tab === "dna" && <><p className="eyebrow">Pattern breakdown</p><h2>The repeatable mechanics</h2><div className="dna-list"><Dna label="Opening promise" text={`The title sets a clear ${video.format.toLowerCase()} before the viewer commits.`} /><Dna label="Audience fit" text={`${video.category} viewers can understand the payoff without extra context.`} /><Dna label="Momentum read" text={`${compact(video.viewsPerHour)} views per hour suggests the signal deserves inspection now.`} /><Dna label="Evidence boundary" text="This is a current-source reading, not a claim of historical growth unless repeated snapshots exist." /></div></>}{tab === "ideas" && <><p className="eyebrow">Creator action</p><h2>What MOMENTUM would do next</h2><div className="idea-list"><Idea title="Convert the hook" text={`Keep the ${video.format.toLowerCase()} but translate the topic for your own niche.`} /><Idea title="Map the audience" text="Use the topic and comment rate to decide whether this is inspiration, tutorial, spectacle, or utility." /><Idea title="Build a 3-video test" text="Create one faithful version, one regional version, and one opposing-take version, then compare retention." /></div><Link href="/ideas" className="primary-button">Open AI Ideas <ChevronRight size={14} /></Link></>}</div>
    <section className="deep-two-column"><div className="panel-surface evidence-table"><div className="section-heading"><div><p className="eyebrow">Observed details</p><h2>What the source says</h2></div></div><div className="detail-rows"><span>Channel</span><strong>{video.channel}</strong><span>Topic</span><strong>{video.topic}</strong><span>Format</span><strong>{video.format}</strong><span>Category</span><strong>{video.category}</strong><span>Category source</span><strong>{video.taxonomySource === "ai" ? "AI taxonomy" : "Rule fallback"}</strong><span>Source</span><strong>{video.sourceMode === "live" ? "YouTube API" : "Sample fixture"}</strong></div><p>{video.categoryReason ?? video.why}</p></div><aside className="panel-surface momentum-next"><p className="eyebrow">Extra layer</p><h2>What we add beyond a trend list</h2><div className="dna-list"><Dna label="Niche translation" text="Rewrite the signal for food, gaming, education, or creator-business niches." /><Dna label="Premium locks" text="City, competitor, export, creator fit, and 30-day history stay locked until backed by repeated evidence." /><Dna label="Action memory" text="Save the signal, turn it into ideas, and return to the same source context later." /></div></aside></section>
