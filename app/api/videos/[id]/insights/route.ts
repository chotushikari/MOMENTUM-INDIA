...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const evidence = { title: video.title, channel: video.channel, views: video.views, likes: video.likes, comments: video.comments, viewsPerHour: video.viewsPerHour, momentumScore: video.momentumScore, evidenceScore: video.evidenceScore, rankConfidence: video.rankConfidence, rankReason: video.rankReason };
  ...
  return NextResponse.json({ mode: "demo", evidence, interpretation: { why: video.why, hook: "Can you do it with only the constraint?", format: video.format, payoff: "A visible final reveal", creatorPlan: buildCreatorActionPlan(video) } });
}
